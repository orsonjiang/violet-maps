...
const DropDown = ({ type, list, currentItem, handleItem }) => {
    ...
    const Exp = (
        <Menu>
            <div className="absolute right-0 my-2 bg-white divide-y divide-gray-100 rounded-lg shadow w-full">
	...
	return (
        <div className="relative">
            <button
                className="flex gap-2 justify-between whitespace-nowrap items-center py-1 px-3 font-medium text-center bg-gray-200 rounded-lg hover:bg-gray-400 focus:outline-none relative w-28"
                onClick={() => {
                    if (menu === type) {
                        dispatch(setMenu(MenuTypes.NONE));
                    } else {
